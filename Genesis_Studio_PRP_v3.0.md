...
| v3.0.1 | 2026-02-11 | 补充图谱非专业可读性详细设计（故事/关系/技术三视图） | 架构组 |
...
#### 5.2.1 图谱可读性详细设计（面向非专业用户）

目标：非专业用户在 5 秒内回答“谁在影响谁、正在发生什么、结果是什么”。

| 视图层 | 默认人群 | 核心问题 | 信息密度 | 交互动作 |
|------|---------|---------|---------|---------|
| 故事视图 (Story) | 业务/运营 | 发生了什么 | 低 | 选中节点后显示自然语言关系句子与关键路径 |
| 关系视图 (Relation) | 分析人员 | 谁影响谁 | 中 | 显示方向关系列表，支持按关键路径过滤 |
| 技术视图 (Technical) | 设计师/开发者 | 如何编辑与追溯 | 高 | 节点/关系增删改、ID 级操作、关系明细 |

设计规则：

1. **渐进披露**：默认进入故事视图，不默认显示长 ID 或底层字段。
2. **自然语言关系**：边标签优先渲染为动词短语（如“正在侦查/正在支援/正在指挥”）。
3. **角色化节点**：每个节点显示“主动/被动/枢纽/独立”角色，降低图论门槛。
4. **关键路径聚焦**：默认仅展示与当前节点直接相关的关键关系，避免全图噪声。
5. **技术能力不下沉**：编辑功能保留在技术视图，避免业务用户误操作。

#### 5.2.2 图谱交互验收标准（UX Acceptance）

| 验收项 | 标准 |
|------|------|
| 5 秒理解测试 | 用户在 5 秒内可描述当前节点的影响方向与关系对象 |
| 关系可解释性 | 至少 80% 的关系以自然语言动词呈现 |
| 操作安全性 | 故事视图与关系视图不暴露破坏性编辑按钮 |
| 视图切换连续性 | 在三视图切换时保持选中节点上下文不丢失 |
| 关键路径可控 | 用户可一键切换“全部关系/关键关系” |
